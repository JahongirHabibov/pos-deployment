...
`/media/<user>/<LABEL>`, и ничего дополнительно не нужно. У киоск-терминалов
сеанса рабочего стола нет, поэтому правило udev из `usb-backup/` монтирует их в
`/mnt/kassio-usb/by-uuid/<UUID>`.
...
Киоск-терминал загружается сразу в браузер без рабочего стола: ни окон, ни меню
